...
def x_u_split(label_per_class, num_classes, labels, include_labeled=True):
    ...
    for i in range(num_classes):
        labeled_ = np.load('./classi/class_{}.npy'.format(i))
...
def get_fixmatch_data(dataset='cifar10', label_per_class=10, batch_size=64, n_iters_per_epoch=1024, mu=7, dist=False,
    ...
    labeled_idx = []
    for i in range(num_classes):
        labeled_idx.append(np.load('./{}_{}_{}/classi/class_{}.npy'.format(args.dataset, args.noise_type, args.noise_rate, i)))
    labeled_idx = np.concatenate(labeled_idx)
